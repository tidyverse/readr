...
test_that("trailing spaces ommitted", {
  ...
  df <- melt_fwf("fwf-trailing.txt", spec, progress = FALSE)
  expect_true(all(df$value == "123"))
})

test_that("respects the trim_ws argument", {
  x <- "a11 b22 c33\nd   e   f  "
  out1 <- melt_fwf(x, fwf_empty(x), trim_ws = FALSE)
  expect_equal(out1$value, c("a11", "b22", "c33", "d  ", "e  ", "f  "))

  out2 <- melt_fwf(x, fwf_empty(x), trim_ws = TRUE)
...
test_that("respects the trim_ws argument with empty fields", {
  ...
  out2 <- melt_fwf(x, fwf_empty(x), trim_ws = TRUE, na = "NA")
...
test_that("fwf_empty can skip comments", {
  ...
  out1 <- melt_fwf(x, fwf_empty(x, comment = "COMMENT"), comment = "COMMENT")
...
test_that("missing lines are not skipped", {
   # first
  x <- "a b\n\n\n1 2"
  expect_equal(max(melt_fwf(x, fwf_empty(x))$row), 4)

   # middle
  x <- "a b\n1 2\n\n\n2 3"
  expect_equal(max(melt_fwf(x, fwf_empty(x))$row), 5)
...
test_that("passing \"\" to melt_fwf's 'na' option", {
  expect_equal(melt_fwf('foobar\nfoo   ', fwf_widths(c(3, 3)), na = "", progress = FALSE)$value,
               c("foo", "bar", "foo", NA))
})
...
test_that("check for line breaks in between widths", {
  ...
  expect_warning(out1 <- melt_fwf(txt1, fwf_empty(txt1)))
  ...
  expect_warning(out2 <- melt_fwf(txt2, fwf_empty(txt2)))
