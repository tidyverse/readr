...
test_that("read_lines standardises line breaks", {
  lf <- read_lines(test_path("eol-lf.csv"), progress = FALSE)
  #expect_equal(read_lines(test_path("eol-cr.csv"), progress = FALSE), lf)
  expect_equal(read_lines(test_path("eol-crlf.csv"), progress = FALSE), lf)
})
