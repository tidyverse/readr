...
test_that("comments are ignored regardless of where they appear", {
  ...
  expect_equal(out1$data_type[2], "integer")
  expect_equal(max(out2$row), 2)
  expect_equal(out3$data_type[2], "integer")

  expect_warning(out4 <- read_csv('x,y\n1,#comment', comment = "#"))
  expect_equal(melt_csv('x,y\n1,#comment', comment = "#")$data_type[4],
               "missing")
  expect_equal(melt_csv('x,y\n1#comment', comment = "#")$data_type[3],
               "integer")
})
...
test_that("melt_csv returns a zero-row data.frame on an empty file", {
...
test_that("_meltsv handles whitespace between delimiters and quoted fields", {
