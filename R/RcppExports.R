# This file was generated by Rcpp::compileAttributes
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

parse_line_delimited <- function(x, delim_ = ",", quote_ = "'", backslash_escape = FALSE, double_escape = FALSE) {
    .Call('fastread_parse_line_delimited', PACKAGE = 'fastread', x, delim_, quote_, backslash_escape, double_escape)
}

#' Read lines
#'
#' @param input Either a path to a file, or a \code{connection}.
#' @param n Number of lines to read. If \code{0}, all lines in the file
#'   will be read.
#' @export
read_lines <- function(input, n = 0L) {
    .Call('fastread_read_lines', PACKAGE = 'fastread', input, n)
}

#' Count the number of lines in a file.
#'
#' @param input Either a path to a file, or a \code{connection}.
#' @param header If \code{TRUE}, subtract one line to account for the
#'   header.
#' @export
count_lines <- function(input, header = FALSE) {
    .Call('fastread_count_lines', PACKAGE = 'fastread', input, header)
}

parse_text <- function(x, what) {
    .Call('fastread_parse_text', PACKAGE = 'fastread', x, what)
}

flip <- function(x) {
    .Call('fastread_flip', PACKAGE = 'fastread', x)
}

read_csv_impl <- function(input, classes, n, header) {
    .Call('fastread_read_csv_impl', PACKAGE = 'fastread', input, classes, n, header)
}

read_csv_impl_filter_pattern <- function(input, classes, n, header, filter_pattern) {
    .Call('fastread_read_csv_impl_filter_pattern', PACKAGE = 'fastread', input, classes, n, header, filter_pattern)
}

#' Read file in a single string
#' 
#' @param path file path name
#' @export
read_file <- function(path) {
    .Call('fastread_read_file', PACKAGE = 'fastread', path)
}

