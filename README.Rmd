...
In the current version, `readr` by default calls `vroom::vroom()` for writing and parsing data.
This means, when you call `readr` you are in effect calling `vroom::vroom()`.

Prior versions of `readr` (< 2.0.0) do not call `vroom::vroom()` and are not as adept at guessing column types.

For most users, the current version of `readr` is preferable.
If you find yourself needing to learn more about how the different `readr` versions parse data, `vignette("column-types")` goes into more detail.
