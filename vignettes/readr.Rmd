...
This will generate a data frame with information about where the problem was found, and why.
...
Since `problems()` gives us the row number from the original file, fixing this value in the original file could be easy, especially since there is only one problematic value.
But data in the wild is rarely so clean.
We can work around this parsing error by importing the date column as character and using the lubridate package to convert that column after the fact.
