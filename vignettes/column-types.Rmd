...
In this so-called second edition, readr calls the vroom package, by default.
...
Note also that the second edition parser (i.e. vroom) sub-samples lines throughout the file, whereas earlier versions of readr consulted the first `guess_max` lines.
...
* Specify `guess_max = Inf`.
  ...
  <!--Put example that triggers the warning here -->
...
* Read all columns as character, then use `type_convert()`.
  ...
  <!-- Consult the issues we've found re: describing the downsides of this. -->
