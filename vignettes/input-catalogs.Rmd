...
library(readr)
library(dplyr)
eg_file <- system.file("extdata/mtcars.csv", package = "readr")
in_catalog <- data_frame(
  in_name = c("first-ten-mtcars", "next-ten-mtcars"),
  in_fun = c("read_csv", "read_csv"),
  in_args = list(list(file = eg_file, n_max = 10L),
                 list(file = eg_file, n_max = 10L, skip = 10L)))
str(in_catalog)
```
...
in_catalog %>% write_rds("input-catalog.rds") # not run
```
